...
async def run_test():
    ...
    executor = Executor()
    memory = MemoryStub()
    orchestrator = Orchestrator(router, planner, executor, memory, vision_model="llava:13b")
    ...
    for i in range(total):
        ...
        # We will patch 'print' so we can capture if vision model says Success: True
        ...
        subprocess.run(f"taskkill /IM {exe_name} /F", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        
        # Special case for calculator on Windows 10/11 which is CalculatorApp.exe
        if app_name == "Calculator":
            subprocess.run("taskkill /IM CalculatorApp.exe /F", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
